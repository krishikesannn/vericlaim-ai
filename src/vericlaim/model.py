...
class VeriClaimModel:
    ...
    def analyze_image(self, source) -> dict[str, object]:
        ...
        # ``review`` is deliberately a very low, high-recall threshold for a
        # broad screening queue.  It must not be used for the normal binary
        # Fraud / Not Fraud result shown to users: that would create a large
        # number of false alerts.  ``classification`` is selected from grouped
        # validation to maximise precision while retaining useful fraud recall.
        screening_threshold = float(self.thresholds["review"])
        classification_threshold = float(self.thresholds.get("classification", screening_threshold))
        classification_threshold_met = probability >= classification_threshold
        return {
            ...
            "image_classification": "Fraud pattern detected" if classification_threshold_met else "Below fraud-classification threshold",
            "review_threshold_met": bool(classification_threshold_met),
            "classification_threshold": round(classification_threshold, 4),
            "broad_screening_threshold": round(screening_threshold, 4),
    ...
    def score_claim(self, image_results: list[dict], claim_amount: float | None = None) -> dict[str, object]:
        ...
        # Use the validated binary threshold for the default claim decision.
        # The lower ``review`` threshold remains available to the UI as a
        # separate broad-screening mode, rather than silently inflating the
        # normal investigation queue.
        model_screening = float(self.thresholds["review"])
        model_review = float(self.thresholds.get("classification", model_screening))
        ...
        return {
            ...
            "policy": {
                ...
                "broad_screening_threshold": round(model_screening * 100, 1),
